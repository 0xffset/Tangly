...
async def register(request_body: RegisterSchema):
    ...
    return ResponseSchema(detail="Enhorabuena! Se ha registrada satisfactoriamente.")


@router.post("/login", response_model=ResponseSchema)
async def login(requset_body: LoginSchema):
    token = await AuthService.logins_service(requset_body)
    return ResponseSchema(detail="Enhorabuena! Ha iniciado satisfactoriamente.", result={"token_type": "Bearer", "access_token": token})
...
async def forgot_password(request_body: ForgotPasswordSchema):
    ...
    return ResponseSchema(detail="Enhorabuena!")